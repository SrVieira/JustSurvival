...
local function onRender ( )
	...
	for player,t in pairs(g_parachuters) do
		if isElement(player) and player ~= localPlayer and getElementData ( player, "parachuting" ) and isElementStreamedIn(player) then
...
function playerQuitWhenParachuting()
	if (isPlayerParachuting(source)) and isElement(getPlayerParachute(source)) then
