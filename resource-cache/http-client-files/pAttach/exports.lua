...
if isClientSide then
    function attachElementToBone(...)
        return pAttach:attachElementToBone(...)
    end
    addEvent("pAttach:attachElementToBone", true)
    addEventHandler("pAttach:attachElementToBone", resourceRoot, attachElementToBone)

    function detachElementFromBone(...)
        return pAttach:detachElementFromBone(...)
    end
    addEvent("pAttach:detachElementFromBone", true)
    addEventHandler("pAttach:detachElementFromBone", resourceRoot, detachElementFromBone)
    ...
    function isElementAttachedToBone(...)
        return pAttach:isElementAttachedToBone(...)
    end

    function getElementBoneAttachmentDetails(...)
        return pAttach:getElementBoneAttachmentDetails(...)
    end
    ...
    function attachElementToBone(element, ped, boneid, ox, oy, oz, rx, ry, rz)
        ...
        return triggerClientEvent("pAttach:attachElementToBone", resourceRoot, element, ped, boneid, ox, oy, oz, rx, ry, rz)
    end

    function detachElementFromBone(element)
        assert(isElement(element), "Expected element at argument 1, got "..type(element))
        cache[element] = nil
        return triggerClientEvent("pAttach:detachElementFromBone", resourceRoot, element)
    end
