...
pAttach = {
    ...
    attachElementToBone = function(self, element, ped, _boneid, ox, oy, oz, rx, ry, rz)
        ...
        if self:isElementAttachedToBone(element) then return false end
    ...
    detachElementFromBone = function(self, element)
        if not self:isElementAttachedToBone(element) then return false end
    ...
    detachAll = function(self, ped)
        ...
        if self.pedInstances[ped] then
            for element in pairs(self.pedInstances[ped].list) do
                self:detachElementFromBone(element)
            end
        end
        return true
    end,

    isElementAttachedToBone = function(self, element)
        return (element and self.instances[element]) and true or false
    end,

    getElementBoneAttachmentDetails = function(self, element)
    ...
    onElementDestroy = function()
        pAttach:detachElementFromBone(source)
    end,
...
addEventHandler("pAttach:receiveCache", resourceRoot, function(cache)
    for _, data in pairs(cache) do
        pAttach:attachElementToBone(unpack(data))
    end
end)
