    ...
    if trackmode == 1 then 
        ...
        for k,v in pairs(Radar_items) do 
            ...
            if screenX ~= false and screenXX ~= false then 
                ...
                dxDrawText ( name, screenX, screenY, screenX, screenY, tocolor(r, g, b), 1, "default", nil, nil, false, false, false, true, false  )            
    ...
    elseif trackmode == 2 then  
        ...
        if screenX ~= false and screenXX ~= false then 
            ...
            if screenXXX ~= false then 
                ...
                dxDrawText ( name, screenXXX+40, screenYYY-15, screenX, screenY, tocolor(r, g, b), 1, "default", nil, nil, false, false, false, true, false )          
                local hp = getElementHealth ( trackedplayer) 
                roundedhp = math.round(hp) 
                if trackedarmor ~= nil then 
                    dxDrawText ( "Nombre: "..getPlayerName(trackedplayer), screenXXX+40, screenYYY, screenX, screenY, tocolor(r, g, b), 1, "default", nil, nil, false, false, false, true, false ) 
...
tracker_Button = {} 
tracker_Grid = {} 
tracker_Window[1] = guiCreateWindow(123,142,223,228,"Localizar jugador",false) 
guiWindowSetSizable(tracker_Window[1],false) 
tracker_Player = guiCreateButton(0.0942,0.8114,0.3901,0.136,"Localizar",true,tracker_Window[1]) 
tracker_All = guiCreateButton(0.5022,0.8158,0.3901,0.1316,"Mostrar a todos",true,tracker_Window[1]) 
...
addEventHandler("onClientPlayerWasted", localPlayer,
	function()
		if guiGetVisible(tracker_Window[1]) then
			guiSetVisible ( tracker_Window[1], false ) 
			showCursor ( false ) 
		end
	end
