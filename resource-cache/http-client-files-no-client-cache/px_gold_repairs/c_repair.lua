...
local assets={
    ...
    textures_paths={
        ...
        "textures/close_icon.png",
        "textures/list.png",
        "textures/nazwa_icon.png",
        "textures/stan_icon.png",
        "textures/koszt_icon.png",
        "textures/row.png",
...
FIX.onRender = function()
    if(FIX.veh and isElement(FIX.veh))then
        ...
        -- header
        dxDrawText("Panel napraw", sw/2-695/2/zoom, sh/2-608/2/zoom, 695/zoom+sw/2-695/2/zoom, sh/2-608/2/zoom+55/zoom, tocolor(200, 200, 200, FIX.alpha), 1, assets.fonts[1], "center", "center")
        local w=dxGetTextWidth("Panel napraw", 1, assets.fonts[1])
        ...
        dxDrawText("Aktualnie naprawiasz: "..getVehicleName(FIX.veh).." #"..(getElementData(FIX.veh, "vehicle:id") or 0), sw/2-695/2/zoom, sh/2-608/2/zoom+608/zoom-50/zoom, 695/zoom+sw/2-695/2/zoom, sh/2-608/2/zoom+608/zoom, tocolor(200, 200, 200,FIX.alpha), 1, assets.fonts[4], "center", "center", false, false, false, true)    
        --
        ...
        onClick(sw/2-694/2/zoom+524/zoom, sh/2-608/2/zoom+608/zoom-50/zoom-57/zoom+(57-39)/2/zoom, 148/zoom, 39/zoom, function()
            if(getPlayerMoney(localPlayer) >= FIX.allCost)then
                if(#fixParts > 0)then
                    ...
                    triggerServerEvent("fix.vehicle", resourceRoot, FIX.veh, FIX.allCost, fixParts)
                else
                    noti:noti("Najpierw zaznacz części.", "error")
                    return
                end
            else
                noti:noti("Brak wystarczających funduszy.", "error")
            end
...
FIX.toggleUI=function(toggle, veh, parts)
    if(toggle)then
        if(getElementData(localPlayer, "user:gui_showed") or FIX.animation)then return end
        ...
        if(#FIX.parts > 0)then
            ...
            blur=exports.blur
            mechanic=exports.px_workshop_mechanic
            ...
            FIX.buttons[1]=buttons:createButton(sw/2-694/2/zoom+524/zoom, sh/2-608/2/zoom+608/zoom-50/zoom-57/zoom+(57-39)/2/zoom, 148/zoom, 39/zoom, "NAPRAW", 255, 10/zoom, false, false, ":px_gold_repairs/textures/button_icon.png")
...
addEvent("FIX.toggleUI", true)
addEventHandler("FIX.toggleUI", resourceRoot, FIX.toggleUI)
