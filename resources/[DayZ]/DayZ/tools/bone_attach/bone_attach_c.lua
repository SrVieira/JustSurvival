...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		...
		elseif isElementStreamedIn(ped) then
			...
			local pedDimension = getElementDimension(ped)
			local pedInt = getElementInterior(ped)
			...
			--if type(objx) == "number" or type(objy) == "number" or type(objz) == "number" then
			if not isNaN(objx) and not isNaN(objy) and not isNaN(objz) then
				--if isElement(element) and objx ~= "NaN" and objy ~= "NaN" and objz ~= "NaN"
					setElementPosition(element, objx, objy, objz)
				--end
			end
			
			--if type(offrx) ~= "number" or type(offry) ~= "number" or type(offrz) ~= "number" then
			if not isNaN(offrx) and not isNaN(offry) and not isNaN(offrz) then
				setElementRotation(element,offrx,offry,offrz,"ZXY")
			end
			
			setElementDimension(element, pedDimension)
			setElementInterior(element, pedInt)
		else
		    local pedDimension = getElementDimension(ped)
			local pedInt = getElementInterior(ped)
			setElementDimension(element, pedDimension)
