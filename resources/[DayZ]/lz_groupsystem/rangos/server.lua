...
local deleteRak = false
function onstarts(  )
	if deleteRak then
    	--local qh = dbQuery( connection,"DROP TABLE groupRanks" )
		--dbFree( qh )
    end
    local qh = dbQuery( connection,"CREATE TABLE IF NOT EXISTS groupRanks ( grupo STRING, rankName STRING, kickP STRING, Pchat STRING, CRank STRING, AAlianzas STRING, Apelaciones STRING, informacion STRING)")
	dbFree( qh )
end
addEventHandler("onResourceStart",getResourceRootElement( getThisResource() ),onstarts)
...
function ClientCreateRank ( nombreRank, kick, chat, Crangos, Alianzas, apelaciones, informacion, bool )
	...
	if not isRankAlreadyCreated ( playerGroup, nombreRank ) then
		addGroupLog( playerGroup, getPlayerName( source ).." ( "..getAccountName( getPlayerAccount( source ) ).." ) Creo un nuevo Rango: "..nombreRank )
		...
		if bool then
			...
			addGroupLog( playerGroup, getPlayerName( source ).." ( "..getAccountName( getPlayerAccount( source ) ).." ) Modifico el Rango: "..nombreRank )
...
function deleteRanks( rankName )
	...
	--
	addGroupLog( playerGroup, getPlayerName( source ).." ( "..getAccountName( getPlayerAccount( source ) ).." ) Elimino el Rango: "..rankName )
	...
	--dbExec( connection, "UPDATE groupmembers SET rang=? WHERE memberacc=? AND name=?", tostring( rang ),acc,gang )
end
