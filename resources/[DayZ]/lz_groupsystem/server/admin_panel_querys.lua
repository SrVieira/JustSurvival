...
function textLabels( group )
	...
	local qh = regis_db:query("SELECT * FROM groupmembers")
	if cosaTextsT[source] then
		cosaTextsT[source] = nil
	end
	for _, columna in ipairs( dbPoll( qh, -1 ) ) do
		local player = getPlayerFromName( columna["membername"] ) 
		if isElement( player ) then
			if isPlayerFounder( player, columna["name"] ) then
				if not cosaTextsT[source] then
					cosaTextsT[source] = string.gsub( columna["membername"], "#%x%x%x%x%x%x", "" )
				else
					cosaTextsT[source] = cosaTextsT[source]..","..string.gsub( columna["membername"], "#%x%x%x%x%x%x", "" )
				end	
			end
		end
	end
	triggerClientEvent( source, "textLabels", source, members, cosaTextsT[source] )
end
...
function getChatRegList(  )
	table.sort( cacheRegistry, function ( a, b )
		return tonumber( a[1] ) > tonumber( b[1] )
	end)
	triggerClientEvent( source, "sendChatRegGPC", source, cacheRegistry )
end
addEventHandler("getGPCChatListG", root, getChatRegList )

function getActRegList(  )
	table.sort( actionS, function ( a, b )
		return tonumber( a[1] ) > tonumber( b[1] )
	end)
	triggerClientEvent( source, "sendACGPCList", source, actionS )
end
addEventHandler("getGPCActiList", root, getActRegList )
