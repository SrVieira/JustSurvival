...
function addChatRegistry( group, playerT, message )
	...
	for _, players in ipairs( getElementsByType("player") ) do
		...
		local cuentaPlayers = players:getAccount();
		if not cuentaPlayers:isGuest() then
...
function alianzaChat( source, cmd, ... )
	...
	if not guestPlayer then
		...
		if isPlayerInGroup( source ) then
			...
			local alianzasList = getGroupAlianzes ( grupo )

			local aN = 0
			for i,v in ipairs( alianzasList ) do
				aN = aN + 1
			end
