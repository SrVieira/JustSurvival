...
GUIEditor.label[1] = guiCreateLabel(195, 185, 96, 15, "Total miembros:", false, tabgrupos)
guiSetFont(GUIEditor.label[1], "default-bold-small")
totalmmlabel = guiCreateLabel(291, 185, 96, 15, "", false, tabgrupos)
guiSetFont(totalmmlabel, "default-bold-small")
GUIEditor.label[2] = guiCreateLabel(195, 210, 67, 15, "Total slots:", false, tabgrupos)
guiSetFont(GUIEditor.label[2], "default-bold-small")
totalslotsllbl = guiCreateLabel(262, 210, 96, 15, "", false, tabgrupos)
guiSetFont(totalslotsllbl, "default-bold-small")
GUIEditor.label[3] = guiCreateLabel(195, 235, 96, 15, "Leaders activos:", false, tabgrupos)
...
--GUIEditor.label[4] = guiCreateLabel(195, 314, 106, 15, "Fecha de creacion:", false, tabgrupos)
...
tabregistro = guiCreateTab("Registros", tabpanel)

gridprreg = guiCreateGridList(10, 36, 398, 245, false, tabregistro)
a1 = guiGridListAddColumn(gridprreg, "hora", 0.22)
a2 = guiGridListAddColumn(gridprreg, "Por", 0.25)
a3 = guiGridListAddColumn(gridprreg, "Descripcion", 0.6)
GUIEditor.label[6] = guiCreateLabel(9, 10, 131, 16, "Registro de acciones", false, tabregistro)
guiSetFont(GUIEditor.label[6], "default-bold-small")
msjregistr = guiCreateButton(10, 291, 79, 39, "Ver registro de mensajes", false, tabregistro)
guiSetFont(msjregistr, "default-bold-small")
guiSetProperty(msjregistr, "NormalTextColour", "FFAAAAAA")
veraccregis = guiCreateButton(104, 291, 79, 39, "Ver registro de acciones", false, tabregistro)
guiSetFont(veraccregis, "default-bold-small")
guiSetProperty(veraccregis, "NormalTextColour", "FFAAAAAA")
close2 = guiCreateButton(198, 291, 79, 39, "Cerrar", false, tabregistro)
guiSetFont(close2, "default-bold-small")
guiSetProperty(close2, "NormalTextColour", "FFAAAAAA")
...
msjgui = guiCreateWindow(402, 72, 525, 457, "Registro de mensajes", false)
guiWindowSetSizable(msjgui, false)
guiSetVisible(msjgui, false )
msjregistgrid = guiCreateGridList(10, 47, 505, 324, false, msjgui)
idc = guiGridListAddColumn(msjregistgrid, "ID", 0.09)
hc = guiGridListAddColumn(msjregistgrid, "Hora", 0.17)
gpc = guiGridListAddColumn(msjregistgrid, "Grupo", 0.12)
nickgp = guiGridListAddColumn(msjregistgrid, "Nickname", 0.12)
msc = guiGridListAddColumn(msjregistgrid, "Mensaje", 0.5)
guiGridListAddRow(msjregistgrid)
guiGridListSetItemText(msjregistgrid, 0, 1, "12", false, false)
guiGridListSetItemText(msjregistgrid, 0, 2, "[14:20:15]", false, false)
guiGridListSetItemText(msjregistgrid, 0, 3, "prueba", false, false)
guiGridListSetItemText(msjregistgrid, 0, 4, "rex", false, false)
guiGridListSetItemText(msjregistgrid, 0, 5, "sadsdasad asd dsassdaasdsdasdasdas  asddsadssd", false, false)
clearregist = guiCreateButton(10, 381, 105, 44, "Limpiar registro      ( Solo admins )", false, msjgui)
guiSetFont(clearregist, "default-bold-small")
guiSetProperty(clearregist, "NormalTextColour", "FFAAAAAA")
cerrar3 = guiCreateButton(125, 381, 105, 44, "Cerrar", false, msjgui)
guiSetFont(cerrar3, "default-bold-small")
guiSetProperty(cerrar3, "NormalTextColour", "FFAAAAAA")
centerWindow(msjgui)


accregisgui = guiCreateWindow(402, 125, 370, 377, "Registro de acciones", false)
guiWindowSetSizable(accregisgui, false)
guiSetVisible(accregisgui, false )
registroaccioneslist = guiCreateGridList(15, 32, 340, 275, false, accregisgui)
hac = guiGridListAddColumn(registroaccioneslist, "hora", 0.27)
pby = guiGridListAddColumn(registroaccioneslist, "Por", 0.3)
aac = guiGridListAddColumn(registroaccioneslist, "Accion", 0.5)
clearregistacc = guiCreateButton(16, 315, 95, 39, "Limpiar registro ( solo admins )", false, accregisgui)
guiSetFont(clearregistacc, "default-bold-small")
guiSetProperty(clearregistacc, "NormalTextColour", "FFAAAAAA")
close4 = guiCreateButton(121, 315, 95, 39, "Cerrar", false, accregisgui)
guiSetFont(close4, "default-bold-small")
guiSetProperty(close4, "NormalTextColour", "FFAAAAAA")
centerWindow(accregisgui)
...
GUIEditor.label[1] = guiCreateLabel(269, 167, 80, 16, "Nickname:", false, guiconfig)
guiSetFont(GUIEditor.label[1], "default-bold-small")
lblnamer = guiCreateLabel(332, 167, 109, 16, "", false, guiconfig)
guiSetFont(lblnamer, "default-bold-small")
GUIEditor.label[2] = guiCreateLabel(269, 193, 80, 16, "Cuenta:", false, guiconfig)
guiSetFont(GUIEditor.label[2], "default-bold-small")
lblcuentac = guiCreateLabel(332, 193, 109, 16, "", false, guiconfig)
guiSetFont(lblcuentac, "default-bold-small")
GUIEditor.label[3] = guiCreateLabel(269, 219, 80, 16, "Rango:", false, guiconfig)
guiSetFont(GUIEditor.label[3], "default-bold-small")
...
--guiComboBoxAddItem(boxranggpc, "Fundador")
--guiComboBoxAddItem(boxranggpc, "Lider")
--guiComboBoxAddItem(boxranggpc, "Aspirante")
...
function enableGPCGui()
    ...
    guiGridListClear( gridprreg )
    guiGridListClear( registroaccioneslist )
...
function viewMessagesRegist(  )
    guiSetVisible( admingrouppanel, false)
    guiSetVisible( msjgui, true)
    guiGridListClear( msjregistgrid )
    triggerServerEvent("getGPCChatListG", getLocalPlayer(  ))
end
addEventHandler("onClientGUIClick", msjregistr, viewMessagesRegist, false)

function addListRegChatGPC( tablee )
    for _, value in ipairs( tablee ) do
        local id, hour, group, nick, message = value[1], value[2], value[3], value[4], value[5]
        local row = guiGridListAddRow( msjregistgrid )
        guiGridListSetItemText( msjregistgrid, row, idc, id, false, false )
        guiGridListSetItemText( msjregistgrid, row, hc, hour, false, false )
        guiGridListSetItemText( msjregistgrid, row, gpc, group, false, false )
        guiGridListSetItemText( msjregistgrid, row, nickgp, nick, false, false )
        guiGridListSetItemText( msjregistgrid, row, msc, message, false, false )
    end
end
addEventHandler( "sendChatRegGPC", getLocalPlayer(  ), addListRegChatGPC )

function closeMessagesRegist(  )
    guiSetVisible( admingrouppanel, true)
    guiSetVisible( msjgui, false)
    guiGridListClear( msjregistgrid )
end
addEventHandler("onClientGUIClick", cerrar3, closeMessagesRegist, false)

function viewACtionsRegist(  )
    guiSetVisible( admingrouppanel, false)
    guiSetVisible( accregisgui, true)
    guiGridListClear( registroaccioneslist )
    guiGridListClear( gridprreg )
    triggerServerEvent("getGPCActiList", getLocalPlayer(  ))
end
addEventHandler("onClientGUIClick", veraccregis, viewACtionsRegist, false)


function addListRegActGPC( tablee )
    for _, value in ipairs( tablee ) do
        local  hour, by, description = value[2], value[3], value[4]

        local row = guiGridListAddRow( registroaccioneslist )
        guiGridListSetItemText( registroaccioneslist, row, hac, hour, false, false )
        guiGridListSetItemText( registroaccioneslist, row, pby, by, false, false )
        guiGridListSetItemText( registroaccioneslist, row, aac, description, false, false )

        local row = guiGridListAddRow( gridprreg )
        guiGridListSetItemText( gridprreg, row, a1, hour, false, false )
        guiGridListSetItemText( gridprreg, row, a2, by, false, false )
        guiGridListSetItemText( gridprreg, row, a3, description, false, false )
    end
end
addEventHandler( "sendACGPCList", getLocalPlayer(  ), addListRegActGPC )

function closeACtionsRegist(  )
    guiSetVisible( admingrouppanel, true)
    guiSetVisible( accregisgui, false)
    guiGridListClear( registroaccioneslist )
    guiGridListClear( gridprreg )
...
addEventHandler("onClientGUIClick", close4, closeACtionsRegist, false)

function clearChatReg()
    triggerServerEvent("clearChatRegS", getLocalPlayer(  ))
    guiGridListClear( msjregistgrid )
    guiGridListClear( registroaccioneslist )
    guiGridListClear( gridprreg )
    triggerServerEvent("getGPCActiList", getLocalPlayer(  ))
...
addEventHandler("onClientGUIClick", clearregist, clearChatReg, false)

function clearActionsReg()
    triggerServerEvent("clearActiRegS", getLocalPlayer(  ))
    guiGridListClear( msjregistgrid )
    guiGridListClear( registroaccioneslist )
    guiGridListClear( gridprreg )
    triggerServerEvent("getGPCActiList", getLocalPlayer(  ))
...
addEventHandler("onClientGUIClick", clearregistacc, clearActionsReg, false)
...
function senddatasSlots ()
    ...
    if ammount:len() >= 2 then
        ...
        outputChatBox(messageG[2],200,0,0,true)
    end
end
...
function deleteAmemberGPC( )
    ...
    if rango ~= "Fundador" then
        ...
        guiGridListClear( configlistmembers )
        triggerServerEvent( "getMembersGPCList", getLocalPlayer(), group_selected )
        outputChatBox(messageG[3], 200, 0, 0, true)
    end
end
...
function aggAmembersOfLis( )
    ...
    if group == "N/A" then
        ...
        outputChatBox(messageG[4],200,0,0,true)
        refreshAGgMemberList()
    end
end
...
function editGPNameGPC(  )
    ...
    if editText:len() > 4 then
        ...
        outputChatBox("Ingresa un nombre mas largo!",200,0,0,true)
    end
end
...
Al dar click al boton verificar si se selecciono algo de la lista, sino retornar con un mensaje
luego almacenar en una variable el grupo seleccionado.
luego mostrar una pequeña gui para insertar en un editbox el nuevo Nombre del grupo.
al dar click en aceptar se debe verificar que el texto ingresado tenga una cantidad de digitos mayor 4.
si se cumple esto mandar a server side el texto del editbox
luego en server side, Hacer un query a la db seleccionando todos los datos desde el nombre anterior del grupo.
definir en una variable el jugador.
definir en una variable la cuenta del jugador.
si es un elemento el jugador entonces
    Hacer un update del nombre del grupo de el jugador desde su cuenta.
    cambiar el elementData.
sino
    Hacer un update del nombre del grupo de el jugador desde su cuenta.
fin
ahora una vez cambiado todo a los jugadores dentro de este clan hacer un update al nombre de este clan.
registrar este acontecimiento.
Avisar que se aplico.
]]--